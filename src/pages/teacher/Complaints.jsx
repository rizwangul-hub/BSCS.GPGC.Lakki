...
const Complaints = () => {
  useDocumentMetadata(
    "Complaints Board | Teacher Portal",
    "Grievance board: reply to student issues and resolve complaints."
  );
  ...
  const { data: complaintsData, loading, refetch: refetchComplaints } = useCachedGet('/teacher/complaints');
  ...
  const handleReplySubmit = async (e) => {
    ...
    try {
      const res = await api.post(`/teacher/complaints/${selectedComplaint._id}/reply`, {
        reply: replyText
      });
      ...
      if (res.data.success) {
        ...
        invalidateCache('/teacher/complaints');
        refetchComplaints();
      }
    } catch (err) {
      ...
      <div className="flex flex-col gap-8 text-left">
        ...
        <div>
          <h1 className="font-heading font-extrabold text-2xl text-secondary">
            Grievance & Complaints Board
          </h1>
          <p className="text-sm text-text-secondary mt-1">
            Review complaints posted by students enrolled in your subjects and reply to resolve issues.
          </p>
        </div>
          ...
          <form onSubmit={handleReplySubmit} className="flex flex-col gap-4 text-left">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                {selectedComplaint.status === 'pending' ? "Draft Reply Text" : "Instructor Reply"}
              </label>
